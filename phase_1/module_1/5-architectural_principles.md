...
1. **Logs** - Logs are text-based records of discrete events. They are the answer to the question, what happend?
2. **Metrics** - Metrics are numerical measurements collected over time. The tell you How much/how many of...?
3. **Traces** - Traces follow a single request as it travels through a distributed system. They allow to know where time was spent
...
class TenantAwareRepository {
  ...
  async findAll() {
    return db.query("SELECT * FROM projects WHERE tenant_id = $1", [
      this.tenantId,
    ]);
  }
